package com.example.androidslimesoccer.single_player.controller;
...
import com.example.androidslimesoccer.SoundManager;
...
public class SinglePlayerLogicProvider {
    ...
    public void doSpecial(SlimeSprite performerSlimeSprite, SpecialSprite specialSprite
        ...
        if (specialSprite.slimeType == SlimeType.INDIAN) {
            ...
            out:
            if (performerSlimeSprite.isMoveLeft) {
    ...
    public void aiUpdateAction() {
        ...
        if (difficulty.toLowerCase().equals("easy")) {
            ...
            Log.i("aiSpriteCenterX", Integer.toString(aiSpriteCenterX));
            Log.i("playerSpriteCeneterX", Integer.toString(playerSpriteCenterX));
            Log.i("ballSpriteX", Integer.toString(ballSprite.x));
            if (aiSpriteCenterX > playerSpriteCenterX && aiSpriteCenterX > ballSprite.x
                ...
                aiGotoBall(false);
                    ...
                    && ballSprite.x - playerSpriteCenterX < aiSpriteCenterX - ballSprite.x) {
                aiGotoNet();
            } else if (ballSprite.x < playerSpriteCenterX) {
                aiGotoBall(false);
            } else {
                aiGotoNet();
            }
            ...
            if (aiSpriteCenterX > playerSpriteCenterX && aiSpriteCenterX > ballSprite.x
                    && playerSpriteCenterX < ballSprite.x
                    && ballSprite.x - playerSpriteCenterX + slimeSprite1.slimeImage.getWidth() / 2 >= aiSpriteCenterX - ballSprite.x) {
                aiGotoBall(true);
            } else if (aiSpriteCenterX > playerSpriteCenterX && aiSpriteCenterX > ballSprite.x
                    && playerSpriteCenterX < ballSprite.x
                    && ballSprite.x - playerSpriteCenterX < aiSpriteCenterX - ballSprite.x) {
    ...
    public void aiGotoBall(boolean isHard) {
        int goalLimitX;
        if (isHard) {
            goalLimitX = slimeSprite1.slimeImage.getWidth() / 2 * (slimeSprite2.x / Utils.rightGoalX);
        } else {
            goalLimitX = Utils.slimeRatio;
        }
        int aiSpriteCenterX = slimeSprite2.x + goalLimitX;
